...
🏗️ 시스템 아키텍처
Zoom 갤러리 화면 캡처 (백그라운드)
        ↓
학생별 타일 Crop (3×3 또는 4×4 Grid)
        ↓
┌─────────────────────────────────┐
│  Feature Loop (10 FPS)          │  ← MediaPipe EAR / MAR / Head Pose 추출
│  → 80프레임 슬라이딩 윈도우 버퍼  │
└─────────────────────────────────┘
        ↓ (8초마다)
┌─────────────────────────────────┐
│  Broadcast Loop (1 FPS)         │  ← CNN+LSTM 모델 추론
│  → 졸음 확률 판정 + 상태 결정    │
└─────────────────────────────────┘
        ↓
FastAPI + WebSocket
        ↓
실시간 대시보드 (new_dashboard.html)
